...
import {
  buildGoogleQuery,
  buildGoogleUrl,
  consumeOneQueryOrThrow,
  getQuotaLocal
} from './services/searchService';
import { TerminalLog } from './components/TerminalLog';
import { isAuthed, loginWithPassword, logout } from './services/authService';
...
const App: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    setAuthLoading(true);
    try {
      const r = await loginWithPassword(pw);
      if (!r.ok) {
  ...
  const handleLoadMore = async () => {
    alert(
      "No MVP (SERP Assistido), 'Carregar mais' significa fazer outra busca no Google (consome quota) com variações de termos."
    );
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative">
      ...
      {quota && (
        <div
          className={`w-full max-w-3xl mb-4 rounded-xl border px-4 py-3 text-sm ${
            quota.remaining === 0
      ...
      {isLocked && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 px-4">
          <div className="w-full max-w-lg rounded-2xl border border-red-500/30 bg-black p-6 text-center">
            ...
            <p className="text-white/50 mt-2 text-sm">(Dica: ao virar o dia, basta recarregar a página.)</p>
          </div>
        </div>
      )}
      ...
      {!authed && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 px-4">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-black p-6">
            ...
            <p className="text-white/70 mt-2 text-sm">Digite a senha para acessar o Starecruiter.</p>
            ...
            <div className="mt-5 space-y-3">
              ...
              <button
                ...
                className="w-full rounded-xl bg-white text-black font-bold py-3 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {authLoading ? 'Validando...' : 'Entrar'}
              </button>

              <div className="text-xs text-white/40">(Sessão salva por 7 dias neste navegador.)</div>
            </div>
          </div>
        </div>
      )}

      {/* IMPORTANT: Only render the app if authed */}
      {authed && (
        <>
          {/* Logout */}
          <button
            onClick={handleLogout}
            className="fixed top-4 left-4 z-[80] px-3 py-2 rounded-xl bg-black text-white text-sm font-bold hover:bg-gray-800"
            title="Sair"
          >
            Sair
          </button>
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 justify-items-center mb-12">
                    ...
                    >
                      ...
                      <span className="text-gray-500 font-semibold group-hover:text-black uppercase tracking-wide">Carregar Mais 5</span>
                    </button>
                  </div>
